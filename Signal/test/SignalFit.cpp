...
string referenceProcTTH_="TTH";
string referenceTagWV_="TTHHadronicTag2";
string referenceTagRV_="TTHHadronicTag2";
...
RooRealVar *mass_;
RooRealVar *vertex_idx_;
RooRealVar *intLumi_;
RooRealVar *tthMVA_RunII; 
RooRealVar *bdtg; 
...
void OptionParser(int argc, char *argv[]){
	...
	desc1.add_options()
		...
		("verbose,v", po::value<int>(&verbose_)->default_value(1),                                			"Verbosity level: 0 (lowest) - 3 (highest)")
...
RooDataSet * reduceDataset(RooDataSet *data0){
	...
	RooDataSet *data = (RooDataSet*) data0->emptyClone()->reduce(RooArgSet(*mass_, *vertex_idx_));
	RooRealVar *weight0 = new RooRealVar("weight","weight",-100000,1000000);
	for (unsigned int i=0 ; i < data0->numEntries() ; i++){
		mass_->setVal(data0->get(i)->getRealValue("mass"));
		weight0->setVal(data0->weight() ); // <--- is this correct?
		vertex_idx_->setVal(data0->get(i)->getRealValue("vertex_idx"));
		data->add( RooArgList(*mass_, *vertex_idx_, *weight0), weight0->getVal() );
	}
	return data;
}
...
void plotBeamSpotDZdist(RooDataSet *data0, string suffix=""){
	...
	for (unsigned int i=0 ; i < data0->numEntries() ; i++){
		...
		weight0->setVal(data0->weight() ); // <--- is this correct?
		vertex_idx_->setVal(data0->get(i)->getRealValue("vertex_idx"));
		if (fabs(vertex_idx_->getVal()) <0.1){
			histSmallDZ->Fill( vertex_idx_->getVal(),data0->weight());
		} else {
			histLargeDZ->Fill( vertex_idx_->getVal(),data0->weight());
		}
	}
...
// split a dataset between right vertex (vertex_idx <1cm ) and wrogn vertex (vertex_idx >1cm)
RooDataSet * rvwvDataset(RooDataSet *data0, string rvwv){

	RooDataSet *dataRV = (RooDataSet*) data0->emptyClone()->reduce(RooArgSet(*mass_, *vertex_idx_));
	RooDataSet *dataWV = (RooDataSet*) data0->emptyClone()->reduce(RooArgSet(*mass_, *vertex_idx_));
	RooRealVar *weight0 = new RooRealVar("weight","weight",-100000,1000000);
	for (unsigned int i=0 ; i < data0->numEntries() ; i++){
		mass_->setVal(data0->get(i)->getRealValue("mass"));
		weight0->setVal(data0->weight() ); 
		vertex_idx_->setVal(data0->get(i)->getRealValue("vertex_idx"));
		if (fabs(vertex_idx_->getVal() )<1.){
			dataRV->add( RooArgList(*mass_, *vertex_idx_, *weight0), weight0->getVal() );
		} else{
			dataWV->add( RooArgList(*mass_, *vertex_idx_, *weight0), weight0->getVal() );
		}
	}
...
RooDataSet * beamSpotReweigh(RooDataSet *data0 /*original dataset*/){
	...
	for (int i = 0; i < data0->numEntries(); i++) {
		...
		if (fabs(vertex_idx_->getVal()) < 0.1 ){
			...
			double mcBeamSpot=TMath::Gaus(vertex_idx_->getVal(),0,TMath::Sqrt(2)*mcBeamSpotWidth_,true); 
			double dataBeamSpot=TMath::Gaus(vertex_idx_->getVal(),0,TMath::Sqrt(2)*dataBeamSpotWidth_,true); 
...
//		mass_->setVal(data0->get(i)->getRealValue("mass"));
		mass_->setVal(data0->get(i)->getRealValue("mass"));
//		tthMVA_RunII->setVal(data0->get(i)->getRealValue("tthMVA_RunII"));
		tthMVA_RunII->setVal(data0->get(i)->getRealValue("tthMVA_RunII"));
		bdtg->setVal(data0->get(i)->getRealValue("BDTG"));
//		vertex_idx_->setVal(data0->get(i)->getRealValue("vertex_idx"));
		weight0->setVal(factor * data0->weight() ); // <--- is this correct?
//		data->add( RooArgList(*mass_, *vertex_idx_, *tthMVA_RunII,*bdtg,*weight0), weight0->getVal() );
		data->add( RooArgList(*mass_, *tthMVA_RunII,*bdtg,*weight0), weight0->getVal() );
	}
...
int main(int argc, char *argv[]){
	...
	// open sig file
	TFile *f1= TFile::Open(filename_[0].c_str());
...
//	TTree *tsub = tall->CopyTree("tthMVA_RunII>0.38 && pho1_idmva>-0.2 && pho2_idmva >-0.2");
	...
	//mass_ = (RooRealVar*)inWS->var("mass");
	mass_ = new RooRealVar("mass","",100,180);
	mass_->SetTitle("m_{#gamma#gamma}");
	mass_->setUnit("GeV");
	//	vertex_idx_ = (RooRealVar*)inWS->var("vertex_idx");
	vertex_idx_ = new RooRealVar("vertex_idx","",0);
	vertex_idx_->setMin(-25.0);
	vertex_idx_->setMax(25.0);
	vertex_idx_->setBins(100);
	//intLumi_ = (RooRealVar*)inWS->var("IntLumi");
//	intLumi_ = new RooRealVar("IntLumi","",1000); 
	intLumi_ = new RooRealVar("IntLumi","",41500); 
	if (!intLumi_) {
	...
	if( mass_ && vertex_idx_ && intLumi_){
		if (verbose_) std::cout << "[INFO] RooRealVars mass, intL and vertex_idx, found ? " << mass_ << ", " << intLumi_  << ", "<< vertex_idx_<<  std::endl;
	} else {
		std::cout << "[ERROR] could not find some of these RooRealVars in WS: mass_ " << mass_ << " vertex_idx " << vertex_idx_ << " intLumi " << intLumi_<< ".  exit(1) "<< std::endl;
		exit(1);
	}

	tthMVA_RunII = new RooRealVar("tthMVA_RunII","",-10,10);
	bdtg= new RooRealVar("BDTG","",-1,1);
	RooRealVar *weight= new RooRealVar("weight","",-100000,100000);
	RooDataSet *data0= new RooDataSet("Data_13TeV","",tsub,RooArgSet(*mass_,*tthMVA_RunII,*bdtg,*vertex_idx_,*weight),"","weight"); 
	...
	for (unsigned int iLine = 0 ; iLine < map_proc_.size() ; iLine++){
		...
		for (int mhIndex=0; mhIndex< massList_.size() ; mhIndex++){
			...
			if (verbose_) std::cout << "[INFO] Old dataset (before intLumi change): " << *data << std::endl;
...
//    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.9675 && tthMVA_RunII<0.9937 &&BDTG<0."); 
    ...
    else if (cat=="TTHHadronicTag1")
//    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.9675 && tthMVA_RunII<0.9937 &&BDTG>0."); 
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.9675  &&BDTG>0."); 
    else if (cat=="TTHHadronicTag2")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.9937 && tthMVA_RunII<0.9971 &&BDTG<0."); 
    else if (cat=="TTHHadronicTag3")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.9937 && tthMVA_RunII<0.9971 &&BDTG>0."); 
    else if (cat=="TTHHadronicTag4")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.9971 && tthMVA_RunII<0.9991 &&BDTG<0."); 
    else if (cat=="TTHHadronicTag5")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.9971 && tthMVA_RunII<0.9991 &&BDTG>0."); 
    else if (cat=="TTHHadronicTag6")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.9991 &&BDTG<0."); 
    else if (cat=="TTHHadronicTag7")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.9991 &&BDTG>0."); 
    else if(cat=="TTHLeptonicTag0")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.8435 &&BDTG<0."); 
    //dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.8435 && tthMVA_RunII<0.9346 &&BDTG<0."); 
    else if (cat=="TTHLeptonicTag1")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.8435 &&BDTG>0."); 
    //dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.8435 && tthMVA_RunII<0.9346 &&BDTG>0."); 
    else if (cat=="TTHLeptonicTag2")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.9346 && tthMVA_RunII<0.9625 &&BDTG<0."); 
    else if (cat=="TTHLeptonicTag3")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.9346 && tthMVA_RunII<0.9625 &&BDTG>0."); 
    else if (cat=="TTHLeptonicTag4")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.9625 && tthMVA_RunII<0.9890 &&BDTG<0."); 
    else if (cat=="TTHLeptonicTag5")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.9625 && tthMVA_RunII<0.9890 &&BDTG>0."); 
    else if (cat=="TTHLeptonicTag6")
...
/*
    ...
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.48&&tthMVA_RunII<0.56&&BDTG<-0.5"); 
    else if (cat=="TTHHadronicTag2")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.56&&BDTG<-0.5"); 
    else if (cat=="TTHHadronicTag3")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII<0.48&&BDTG<0&&BDTG>-0.5"); 
    else if (cat=="TTHHadronicTag4")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.48&&tthMVA_RunII<0.56&&BDTG<0&&BDTG>-0.5"); 
    else if (cat=="TTHHadronicTag5")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.56&&BDTG<0&&BDTG>-0.5"); 
    else if(cat=="TTHHadronicTag6")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII<0.48&&BDTG<0.5&&BDTG>0"); 
    else if (cat=="TTHHadronicTag7")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.48&&tthMVA_RunII<0.56&&BDTG<0.5&&BDTG>0"); 
    else if (cat=="TTHHadronicTag8")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.56&&BDTG<0.5&&BDTG>0"); 
    else if (cat=="TTHHadronicTag9")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII<0.48&&BDTG>0.5"); 
    else if (cat=="TTHHadronicTag10")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.48&&tthMVA_RunII<0.56&&BDTG>0.5"); 
    else if (cat=="TTHHadronicTag11")
    dataFull = (RooDataSet*)data->reduce("tthMVA_RunII>0.56&&BDTG>0.5"); 
    */

//			if(cat=="TTHHadronicTag0")
//				dataFull = (RooDataSet*)data->reduce(Cut("tthMVA_RunII<0.48&&BDTG<0")); 
//			else if (cat=="TTHHadronicTag1")
//				dataFull = (RooDataSet*)data->reduce(Cut("tthMVA_RunII>0.48&&tthMVA_RunII<0.56&&BDTG<0")); 
//			else if (cat=="TTHHadronicTag2")
//				dataFull = (RooDataSet*)data->reduce(Cut("tthMVA_RunII>0.56&&BDTG<0")); 
//			else if (cat=="TTHHadronicTag3")
//				dataFull = (RooDataSet*)data->reduce(Cut("tthMVA_RunII<0.48&&BDTG>0")); 
//			else if (cat=="TTHHadronicTag4")
//				dataFull = (RooDataSet*)data->reduce(Cut("tthMVA_RunII>0.48&&tthMVA_RunII<0.56&&BDTG>0")); 
...
//				dataFull = (RooDataSet*)data->reduce(Cut("tthMVA_RunII>0.56&&BDTG>0")); 
			...
			dataRV = rvwvDataset(dataFull,"RV"); 
			dataWV = rvwvDataset(dataFull,"WV"); 

			if (verbose_) std::cout << "[INFO] Datasets ? " << *data << std::endl;
			if (verbose_) std::cout << "[INFO] Datasets cat ? " << *dataFull << std::endl;
