...
void Packager::packageOutput(bool split, string process , string tag){
	...
	std::cout<<"number of cat "<<nCats_<<std::endl;
	for (int cat=0; cat<nCats_; cat++){
...
//			WS->Print();
			...
			std::cout<<"getting norm"<<std::endl;
		...
		std::cout<<"importing pdf"<<std::endl;
	}
	...
	if (runningNormSum->getSize()==0){
		...
		if (!skipPlots_) {
			...
			std::cout<<"what a shit cod3"<<std::endl;
